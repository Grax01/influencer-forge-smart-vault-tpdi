...
async function submitTask(body: any) {
  ...
  if (!submitRes.ok) {
    ...
    // Return 200 with error details instead of proxying upstream status codes.
    // This prevents fetchWrapper from triggering the parent-iframe error banner
    // for agent-level failures (e.g. bad request, upstream 500).
    return NextResponse.json({
      success: false,
      response: { status: 'error', result: {}, message: errorMsg },
      error: errorMsg,
...
async function pollTask(task_id: string) {
  ...
  if (!pollRes.ok) {
    ...
    // Return 200 with error details so fetchWrapper doesn't trigger parent error banner
    return NextResponse.json({
      success: false,
      status: 'failed',
      error: msg,
  ...
  // Task failed — return 200 with error details so fetchWrapper doesn't
  // trigger the parent-iframe error banner (500 is reserved for real server errors)
